...
BRANCH_10="opbsd/10-stable/master"
BRANCH_current="opbsd/current/master"
...
SOURCES_DIR="/usr/data/release/source/git/opBSD"
LOCK_DIR="${SOURCES_DIR}"
LOCK_FILE="${LOCK_DIR}/opbsd-stable-repo-lock"
SOURCES_REPO="/usr/data/source/git/opBSD"
HARDENEDBSD_STABLE_DIR="${SOURCES_DIR}/opBSD.git"
...
WWW_RELEASE_DIR="${WWW_BASE}/pub/opBSD/releases/amd64/amd64"
WWW_ISO_DIR="${WWW_BASE}/pub/opBSD/releases/amd64/amd64/ISO-IMAGES"
...
publish_release()
{
	...
	then
		...
		cat ${LOG_FILE_SHORT} | mail -s "[DONE] opBSD ${_branch} ${_hbsd_date_tag} ${_hbsd_name_tag} RELEASE builds @${DATE}" op@hardenedbsd.org
	else
		cat ${LOG_FILE_SHORT} | mail -s "[FAILED] opBSD ${_branch} ${_hbsd_date_tag} ${_hbsd_name_tag} RELEASE builds @${DATE}" op@hardenedbsd.org
	fi
}
